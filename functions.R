...
template_xref <- tribble(
  ...
  "Zone",                                                   "Zone",              "utm_zone",
  "Datum",                                                  "Datum",             "datum",
  "Coordinate Type (DD, DMS, DDM, UTM, or SP)",             "Coord Type",        "coord_type",
  "Location uncertainty (if known)",                        "Loc. Uncertainty",  "loc_accuracy",
...
common_to_scientific <- function(et, common_name) {
  if(is.na(common_name)) {
    return(NA)
  }
  et$NAME[which(et$COMMON_NAME == common_name)]
}
...
scl_read_template <- function(file) {
  ...
  if(grepl("\\.csv$", file)) {
    ...
    if(template == "db"){
      out <- readr::read_csv(
        file,
        # col_types = "c"
        col_types = cols(obs_date = 'c', obs_date2 = 'c', num_observed = 'c', loc_y = 'c', loc_x = 'c', .default = 'c')
      )
    }
    if(template == "scl"){
  ...
  if(grepl("\\.xlsx?$", file)) {
    header_names <- names(readxl::read_excel(file, sheet = "Report Form", n_max = 0))
    template <- check_which_template(header_names)
    if(template == "db"){
      # unlikely that pod template is in xlsx
    }
    if(template == "scl"){
      out <- read_excel(file, col_types = 'text', sheet = "Report Form")
      names_in_order <- template_xref$database_name[order(match(template_xref$template_name, names(out)))]
...
process_template <- function(df, is_excel = TRUE) {
  out <- df %>% 
    mutate(name_used = case_when(
      !is.na(scientific_name_used) ~ scientific_name_used,
      TRUE ~ common_name_used
...
# TODO - need to simplify taxonomy checking to the "species" column
generate_errors <- function(df) {
  tax_sci_provided <- map2(df$scientific_name_used, df$common_name_used, check_taxonomy_provided)
  ...
  tax_est <- map(df$species, check_taxonomy_in_est)
  tax_synonym  <- map(df$species, check_taxonomy_synonym)
  ...
  a <- pmap(
    list(
      tax_sci_provided, tax_exists, tax_est, tax_synonym,
...
provide_alternative_taxonomy <- function(errors) {
  unique_suggestions <- errors %>% map_df(rbind) %>% filter(code == 101, !duplicated(.))
  unique_synonymy <- errors %>% map_df(rbind) %>% filter(code %in% c(103, 104), !duplicated(.))
  
  print(unique_suggestions)
  print(unique_synonymy)
  
  tax_suggest <- map2(unique_suggestions$value, unique_suggestions$column, function(x,y) {
    if(y=="Scientific Name") {
      suggest_taxonomy(x)
    } else {
      suggest_taxonomy_common(x)
    }
  })
  
  syn_suggest <- map2(unique_synonymy$value, unique_synonymy$column, function(x,y) {
    if(y=="Scientific Name") {
      suggest_synonym(x)
    } else {
      suggest_synonym_common(x)
    }
  })
